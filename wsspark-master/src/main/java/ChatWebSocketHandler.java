...
public class ChatWebSocketHandler {
    ...
    public void onMessage(Session user, String message) {
        ...
        try {
            HttpResponse<JsonNode> response = Unirest.get("https://ghibliapi.herokuapp.com/films").asJson();
            JSONArray array = response.getBody().getArray();
            StringBuilder str = new StringBuilder();
            for (int i = 0; i < array.length(); i++) {
                if(array.getJSONObject(i).getString("title").toLowerCase().contains(pesan)) {
                    str.append(array.getJSONObject(i).getString("title"));
                    str.append(", ");
                }
            }
